    ...
    try:
        ...
        if format == 'pdf':
            ...
            for oficio in oficios_list:
                ...
                if assignments:
                    for assignment in assignments:
                        tecnico_name = user_lookup.get(assignment['tecnico'], assignment['tecnico'])
                        data.append([
                            oficio['id_secuencial'],
                            oficio['numero_oficio'],
                            oficio['gad_parroquial'],
                            oficio['canton'],
                            oficio['estado'].capitalize(),
                            tecnico_name,
                            assignment['tipo_asesoria'],
                            assignment.get('sub_estado', 'Asignado')
                        ])
                else:
                    data.append([
                        oficio['id_secuencial'],
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f'informe_seguimiento_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf'
            )
            ...
            for oficio in oficios_list:
                ...
                if assignments:
                    for assignment in assignments:
                        tecnico_name = user_lookup.get(assignment['tecnico'], assignment['tecnico'])
                        excel_data.append({
                            'ID Secuencial': oficio['id_secuencial'],
                            'Número Oficio': oficio['numero_oficio'],
                            'Parroquia': oficio['gad_parroquial'],
                            'Cantón': oficio['canton'],
                            'Estado': oficio['estado'].capitalize(),
                            'Técnico': tecnico_name,
                            'Tipo Asesoría': assignment['tipo_asesoria'],
                            'Sub-Estado': assignment.get('sub_estado', 'Asignado'),
                            'Fecha Asesoría': format_date_for_traditional(assignment.get('fecha_asesoria', '')),
                            'Desarrollo Actividad': assignment.get('desarrollo_actividad', ''),
                            'Entrega Recepción': assignment.get('entrega_recepcion', 'No Aplica'),
                            'Anexos': 'Sí' if assignment.get('anexo_nombre') else 'No'
                        })
                else:
                    excel_data.append({
                        'ID Secuencial': oficio['id_secuencial'],
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f'informe_seguimiento_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
            )
